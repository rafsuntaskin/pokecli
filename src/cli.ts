...
async function updateAutoResumeMessage(message: string): Promise<void> {
...
function parseGlobalArgs(args: string[]): { args: string[]; message?: string } {
  const rest: string[] = [];
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "-m" || arg === "--message") {
      ...
      if (!next) throw new Error(`${arg} requires a message.`);
      message = next;
      index += 1;
      continue;
    }
